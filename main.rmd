...
# TODO:
- Format Covid data to make it easy to graph over time (we are going to be comparing covid data over time, but for influenza data we'll use one date *2019-2020 I think*). We only need one snapshot of influenza since it's simply to gauge "vaccine willingness" shortly before covid. We want to show Covid data over time as we might find that the correlation is stronger/weaker depending on time as there was a lot of drama over it when it came out that died down over time.
- Determine correlation coefficient between covid and influenza vaccine percentage
- Currently, the county maps display covid and influenza vaccination percentages. Most importantly we need to graph the correlation coefficient on a county map, as that should be our main takeaway. We may be able to graph other data findings we have.
- Less important, but some counties are very small and as such may be outliers (small counties often have 100% or 0% vaccination rates). Perhaps we could import a dataframe of counties and their populations, and use it to filter out counties that have populations too low from our data.
...
It is important to point out that this data is estimated based off of state data and population distribution, meaning the actual percentage of each county may not be accurate. Unfortunately, we do not have the same amount of precision as we do with the covid data as there were less resources dedicated to influenza.
We are only focusing on 2019-2020 influenza data because we just need a snapshot of how willing people are to taking vaccines shortly before COVID-19.
```{r}
...
The covid data contains a lot of information irrelevant to our investigation.
```{r}
...
# Basic Plot (Latest)
This scatterplot seems to demonstrate some correlation between infl and covid vaccination but the relationship appears very weak
There are a considerable amount of counties that report a near 100% vaccination rate, they are likely low population counties.
```{r}
vaccine_data_latest %>%
...
```{r}
library(usmap)
vaccine_data_latest_corrected = rename(vaccine_data_latest, fips = FIPS) # For some reason the map library requires the fips column name to be lowercase
