...
influenza_vaccine_data_cleaned = influenza_vaccine_data %>%
  filter(Vaccine == "Any Influenza Vaccination, Seasonal or H1N1") %>% # All types of infl. vaccinations are counted
  ...
  mutate(`Estimate (%)` = as.numeric(`Estimate (%)`)) # Convert the estimate % from character to numeric
  #mutate(`Season/Survey Year` = as.Date("Season/Survey Year", ))
  select(FIPS, `Estimate (%)`,`Season/Survey Year`) %>% # Filter out irrelevant columns
  filter(`Season/Survey Year` == "2019-20") %>% # Only keep 2019-2020 data for simplicity (may add more dates later)
  rename(Influenza_Percentage = as.numeric(`Estimate (%)`)) %>%
...
# Combine Dataframes
```{r}
vaccine_data = merge(influenza_vaccine_data_cleaned, covid_vaccine_data_cleaned, by = c("FIPS"), all = TRUE)
vaccine_data = select(vaccine_data,FIPS, Influenza_Percentage, Covid_Percentage)
print(vaccine_data)
```
